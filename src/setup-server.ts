 ...
 *
 * It exists so the package is introspectable without a secret: MCP registries,
 * directory scanners and `npx` tyre-kickers can start it, read `tools/list`,
 * and see what the package is for. Unlike a credentialed run it advertises only
 * the `tools` capability — claiming prompts and resources it cannot serve would
 * make introspection lie.
 */
export const createSetupServer = (url: URL): McpServer => {
  const server = new McpServer(
    ...
    {
      capabilities: { tools: {} },
      instructions: `This Mencoro MCP bridge has no credential configured. Call "${SETUP_TOOL_NAME}" for setup instructions.`,
    }
  );
